...
from .forms import ChiefComplaintForm

from django.shortcuts import render, redirect
from .models import Patient, ChiefComplaint
from .forms import ChiefComplaintForm

def add_chief_complaint(request, patient_id):
    patient = Patient.objects.get(pk=patient_id)
    chief_complaints = ChiefComplaint.objects.filter(patient=patient)
    
    if request.method == 'POST':
        form = ChiefComplaintForm(request.POST)
        if form.is_valid():
            chief_complaint_text = form.cleaned_data['chief_complaint_text']
            history_presenting_complaint_text = form.cleaned_data['history_presenting_complaint_text']
            
            # Create a new ChiefComplaint object and link it to the patient
            ChiefComplaint.objects.create(
                patient=patient,
                chief_complaint_text=chief_complaint_text,
                history_presenting_complaint_text=history_presenting_complaint_text
            )
            return redirect('add_chief_complaint', patient_id=patient_id)  # Redirect to the same page
    else:
        form = ChiefComplaintForm()

    return render(request, 'today/add_chief_complaint.html', {'form': form, 'patient': patient, 'chief_complaints': chief_complaints})


def myconsoltantpage(request):
    current_user = request.user
    current_doctor = get_object_or_404(Doctor, user=current_user)
    patient_appointment = MedicalHistoryy.objects.filter(doctor=current_doctor)
    ...
    return render(request,'today/myconsoltantpage.html', context)


from django.shortcuts import render, get_object_or_404
from .models import Doctor, MedicalHistoryy  # Import your models

def patients_doctorlist(request):
    current_user = request.user
    current_doctor = get_object_or_404(Doctor, user=current_user)
    specialization = DoctorSpecialization.objects.filter(doctor=current_doctor)
    ...
    return render(request, 'today/patientlist.html', context)
...
from django.shortcuts import render, get_object_or_404, redirect
from .models import Patient, ChiefComplaint, HospitalHistory
from .forms import ChiefComplaintForm, HospitalHistoryForm
from .models import ChiefComplaint, HospitalHistory
from .forms import ChiefComplaintForm, HospitalHistoryForm
from django.shortcuts import render, get_object_or_404, redirect

def enter_chief_complaint(request, patient_id):
    patient = get_object_or_404(Patient, pk=patient_id)

    if request.method == 'POST':
        form = ChiefComplaintForm(request.POST)
        if form.is_valid():
            chief_complaint = form.save(commit=False)
            chief_complaint.patient = patient
            chief_complaint.save()
            return redirect('patient_details', patient_id=patient_id)
    else:
        form = ChiefComplaintForm()

    saved_chief_complaints = ChiefComplaint.objects.filter(patient=patient)

    context = {
        'form': form,
        'patient': patient,
        'saved_chief_complaints': saved_chief_complaints,
    }

    return render(request, 'today/enter_chief_complaint.html', context)

def enter_hospital_history(request, patient_id):
    patient = get_object_or_404(Patient, pk=patient_id)

    if request.method == 'POST':
        form = HospitalHistoryForm(request.POST)
        if form.is_valid():
            hospital_history = form.save(commit=False)
            hospital_history.patient = patient
            hospital_history.save()
            return redirect('patient_details', patient_id=patient_id)
    else:
        form = HospitalHistoryForm()

    saved_hospital_histories = HospitalHistory.objects.filter(patient=patient)

    context = {
        'form': form,
        'patient': patient,
        'saved_hospital_histories': saved_hospital_histories,
    }

    return render(request, 'today/enter_hospital_history.html', context)
